"""
RC5P encryption implementation with window interface.
The window interface will be available later.
"""
...
rounds = 12
"""
VARIANT OF TYPES ENCRYPTION/DECRYPTION FOR VARIABLE -> rc5_type:
                  1 - rc5p;
                  2 - rc5ra;
"""
rc5_type = 1
...
def encryption_rc5p(text: str or bytes, s: int()) -> bytes:
    ...
    try:
        ...
        print("Ошибка зашифровки. Попробуйте изменить начальные параметры 🙃")
...
def decryption_rc5p(text: str or bytes, s: int()) -> bytes:
    ...
    try:
        ...
        print("Ошибка расшифровки. Попробуйте изменить начальные параметры 🙃")
...
def encryption_rc5ra(text: str or bytes, s: int()) -> bytes:
    ...
    b = (b + s[1]) % 2 ** w
    for i in range(1, rounds + 1):
        # a = (rot_l(a ^ b, func_for_lo(lo[i % c]), w) + s[2 * i]) % 2 ** w
        # b = (rot_l(a ^ b, func_for_lo(lo[i % c]), w) + s[2 * i + 1]) % 2 ** w
...
def decryption_rc5ra(text: str or bytes, s: int()) -> bytes:
    ...
    b = int.from_bytes(text[w_8:], byteorder='little')
    for i in range(rounds, 0, -1):
        # b = rot_r((b % 2 ** w) - s[2 * i + 1], func_for_lo(lo[i % c]), w) ^ a
        # a = rot_r((a % 2 ** w) - s[2 * i], func_for_lo(lo[i % c]), w) ^ b
...
def encryption_text(s: int()) -> None:  # работа с файлами
...
def decryption_text(s: int()) -> None:
    with open(de_inp_file_name, 'rb+') as inp, open(de_out_file_name, 'wb+') as out:
        ...
        while run:
            ...
            # print(text.decode(encoding='utf-8', errors='ignore'))
            out.write(text)
